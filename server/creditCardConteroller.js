...
module.exports = function(app){
    ...
    app.get("/api/creditcards",(req,res)=>{
        ...
        var creditCardsRef = database.ref('/creditCards');
        var query = creditCardsRef.orderByChild('name').equalTo('region');

        query.once('value',snap=>{

            outputs = snap.val(); res.send(outputs); });
            
        });


        // firebase.database().ref('/creditCards').once('value').then(function(snapshot) {
